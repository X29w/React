...
let workInProgressHook: React.Hook | null = null; // 当前正在处理的hook
let currentHook: React.Hook | null = null; // 更新的时候数据来源
let renderLane: React.Lane = NoLane;
const { currentDispatcher } = internals;

export function renderWithHooks(wip: FiberNode, lane: React.Lane) {
  // 赋值操作
...
function pushEffect(
  ...
  const updateQueue = fiber.updateQueue as React.FCUpdateQueue<any>;
  if (updateQueue === null) {
...
function createFCUpdateQueue<State>() {
  const updateQueue =
    createUpdateQueue<State>() as unknown as React.FCUpdateQueue<State>;
...
function updateState<State>(): [State, React.Dispatch<State>] {
  ...
  // 计算新的state逻辑
  const queue = hook.updateQueue as React.UpdateQueue<State>;
  const baseState = hook.baseState;
  const current = currentHook as React.Hook;
...
function updateWorkInProgressHook(): React.Hook {
  ...
  currentHook = nextCurrentHook as React.Hook;
 ...
 * bailout重置变量
 */
export function bailOutHook(wip: FiberNode, renderLane: React.Lane) {
