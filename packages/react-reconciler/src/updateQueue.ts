import { isSubsetOfLanes,  mergeLanes, NoLane } from "./fiberLanes";
...
export const processUpdateQueue = <State>(
  ...
  memoizedState: State;
  baseState: State;
  baseQueue: React.Update<State> | null;
} => {
  ...
  if (pendingUpdate !== null) {
    ...
    let pending = pendingUpdate.next as React.Update<any>;

    let newBaseState = baseState; // 最后一个没被跳过update计算后的结果
    let newBaseQueueFirst: React.Update<State> | null = null;
    let newBaseQueueLast: React.Update<State> | null = null;
    ...
    do {
      ...
      if (!isSubsetOfLanes(renderLane, updateLane)) {
        ...
        if (newBaseQueueFirst === null) {
          ...
          // last u1
          // 不是第一个被跳过的
          (newBaseQueueLast as React.Update<State>).next = clone;
          newBaseQueueLast = clone;
        }
      } else {
        // 优先级足够
      ...
      pending = pending.next as React.Update<any>;
    } while (pending !== first);
