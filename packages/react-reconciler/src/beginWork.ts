...
import { includeSomeLanes, NoLanes } from "./fiberLanes";
import {
  ChildDeletion,
  DidCapture,
  NoFlags,
 ...
 */
export const beginWork = (wip: FiberNode, renderLane: React.Lane) => {
 ...
 * @param renderLane
 */
function bailoutOnAlreadyFinishedWork(wip: FiberNode, renderLane: React.Lane) {
  // 1. 检查优化程度
  /**
   * 如果这个检查返回false，
...
function updateHostRoot(wip: FiberNode, renderLane: React.Lane) {
  ...
  const updateQueue = wip.updateQueue as UpdateQueue<React.ReactElementType>;
  // 这里是计算最新值
