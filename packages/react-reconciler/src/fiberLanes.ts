...
export function mergeLanes(laneA: React.Lane, laneB: React.Lane): React.Lanes {
  return laneA | laneB;
}
 ...
 * @returns {Lane}
 */
export function getHighestPriorityLane(lanes: React.Lanes): React.Lane {
  return lanes & -lanes;
}
 ...
 * 判断优先级是否足够更新(交集）
 */
export function isSubsetOfLanes(set: React.Lanes, subset: React.Lane) {
  return (set & subset) === subset;
}

export function markRootFinished(root: FiberRootNode, lane: React.Lane) {
 ...
 * @param lanes
 */
export function lanesToSchedulerPriority(lanes: React.Lanes) {
 ...
 * @param suspendedLane
 */
export function markRootSuspended(root: FiberRootNode, suspendedLane: React.Lanes) {
 ...
 */
export function markRootPinged(root: FiberRootNode, pingedLane: React.Lanes) {
  root.pingedLanes |= root.suspendedLanes & pingedLane;
}

export function getNextLane(root: FiberRootNode): React.Lane {
 ...
 * @param subset
 */
export function includeSomeLanes(set: React.Lanes, subset: React.Lane | React.Lanes): boolean {
 ...
 * @param subset
 */
export function removeLanes(set: React.Lanes, subset: React.Lane | React.Lanes): React.Lanes {
  return set & ~subset;
}
