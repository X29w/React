...
2. **子节点的协调**：
   ...
   - `ChildReconciler` 函数负责处理子节点的协调逻辑，包括删除、更新和插入子节点。它通过比较当前 Fiber 和新的子节点来决定如何处理。

3. **双缓存树原理**：

   - 代码中使用了双缓存树的原理，通过 `useFiber` 函数基于当前的 FiberNode 创建一个新的 FiberNode，而不需要调用 `new FiberNode`。这有助于提高性能和减少内存开销。

4. **Fragment 的处理**：

   - 代码中处理了 React Fragment 的逻辑，允许开发者在不增加额外 DOM 节点的情况下组合多个子节点。
